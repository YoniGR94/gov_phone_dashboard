...
- הסבר בעברית בריחוף (hover) על אייקון ⓘ בפינת כל גרף וכל נתון פחות מובן-מאליו
- סימון מכשירים שהוצאו ממכירה ואינם ניתנים לרכישה כמכשיר חדש
- כפתור שיתוף צף (Web Share API, עם נפילה לקישור וואטסאפ בדפדפנים ללא תמיכה)
...
בוחרים קבוצת דרגה (אקדמאים, מינהלי, צה"ל וכו'), דרגה, ומכשיר. האתר ממפה אוטומטית את הדרגה למכסת ההשתתפות המתאימה, ומסמן מכשירים שהוצאו ממכירה.

### שלב 2 — לוח המחוונים
מציג את התשלום החודשי של העובד, השתתפות המשרד, ניצול המכסה, מחיר רכישה בסוף התקופה, עלות מצטברת על פני זמן, סימולציית סיום התקשרות מוקדם (עם/בלי רכישת המכשיר), והשוואה בין מכשירים. לצד הגרפים והנתונים המורכבים יותר מופיע אייקון ⓘ עם הסבר בעברית ב-hover.
...
├── src/
│   ├── components/          # DeviceSelector, GradeSelector, Charts, SummaryCards, InfoTooltip, ShareButton, Credit
